...
code_language_mapping = {code: language for language, code in language_code_mapping.items()}
...
def decode_predictions(predictions, source_tokenizer, target_tokenizer):
    ...
    for sentence_prediction, sentence_segmentation in zip(sentence_predictions, sentence_segmentations):
        decoded_sentence_prediction = [
            target_tokenizer.lookup_tokens(word_predictions) for word_predictions in sentence_prediction
        ]
        ...
        if sentence_segmentation is not None:
            decoded_sentence_segmentation = [
                ["".join(source_tokenizer.lookup_tokens(morpheme_indices)) for morpheme_indices in token_indices]
...
def write_predictions(predictions, language_code: str, track: int, model_type: str,
                      trial: int = 1, base_path: str = "./predictions", data_path: str = "./data"):
    os.makedirs(base_path, exist_ok=True)

    predictions_iterator = iter(predictions)
    prediction_file_name = os.path.join(
        "./predictions", f"{language_code}_track{track}_{model_type}_trial{trial}.prediction"
    )
    test_file_name = _make_test_path(language=code_language_mapping[language_code], track=track, data_path=data_path)
...
def get_predictions(path_to_model: str, language: str, track: int, model_type: str, data_path: str = "./data",
    ...
    if model_type == "ctc":
        ...
        model = MorphemeGlossingModel.load_from_checkpoint(checkpoint_path=path_to_model)
    else:
    ...
    # Train Model
    trainer = Trainer(
        accelerator="cpu", devices=1, enable_progress_bar=True, enable_model_summary=verbose, logger=False
    )

    # Get predictions
    predictions = trainer.predict(model=model, dataloaders=dm.test_dataloader())
    predictions = decode_predictions(predictions, dm.source_tokenizer, dm.target_tokenizer)
    return predictions
...
def get_predictions_from_retrained_models(base_path: str = "./retrain_results"):
    for retrained_model in os.listdir(base_path):
        ...
        best_checkpoint = [
            checkpoint for checkpoint in os.listdir(path_to_saved_models) if checkpoint != "last.ckpt"
        ]
